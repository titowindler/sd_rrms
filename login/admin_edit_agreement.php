                ...
                <div class="card">
                  <div class="card-body">
                   ...
                   <?php 
                      ...
                      $sqlAgreement = "SELECT * FROM agreement WHERE agreement.tenantID = '$tenant_id' ";
                      $resultAgreement = mysqli_query($conn,$sqlAgreement);

                      while($rowAgreement = mysqli_fetch_assoc($resultAgreement)) {

                        $agreementID = $rowAgreement['agreementID'];
                        $name_of_occupant = $rowAgreement['name_of_occupant'];
                        $address = $rowAgreement['address'];
                        $emailaddress  = $rowAgreement['emailaddress'];
                        $city = $rowAgreement['city'];
                        $mobile_number = $rowAgreement['mobile_number'];
                        $rental_rate = $rowAgreement['rental_rate'];
                        $date_occupied = $rowAgreement['date_occupied'];
                        $collection_of_rental = $rowAgreement['collection_of_rental'];
                 ...
                 <div class="card-body">
                    <form class="forms-sample" action="../function/agreement.php" method="POST">
                      <div class="form-group">
                        ...
                        <input type="hidden" value="<?php echo $agreementID?>" name="agreementID">

                        <input type="hidden" value="<?php echo $tenant_id?>" name="tenantID">

                        <label for="exampleInputUsername1">Name of Occupant</label>
                        <input type="text" class="form-control" id="exampleInputUsername1" value="<?php echo $name_of_occupant ?>" name="name_of_occupant">
                      </div>
                      <div class="form-group">
                        <label for="exampleInputEmail1">Address</label>
                        <input type="text" class="form-control" id="exampleInputEmail1" value="<?php echo $address ?>" name="address" >
                      </div>
                      <div class="form-group">
                        <label for="exampleInputEmail1">Email Address</label>
                        <input type="email" class="form-control" id="exampleInputEmail1" value="<?php echo $emailaddress ?>" name="emailaddress" >
                      </div>
                      <div class="form-group">
                        <label for="exampleInputPassword1">Provincial / City</label>
                        <input type="text" class="form-control" id="exampleInputPassword1" value="<?php echo $city ?>" name="city">
                      </div>
                      <div class="form-group">
                        <label for="exampleInputConfirmPassword1">Telephone / Mobile</label>
                        <input type="text" class="form-control" id="exampleInputConfirmPassword1" value="<?php echo $mobile_number ?>" name="mobile_number">
                      </div>
                       <div class="form-group">
                        <label for="exampleInputConfirmPassword1">Rental Rate</label>
                        <input type="text" class="form-control" id="exampleInputConfirmPassword1" value="<?php echo $rental_rate ?>" name="rental_rate">
                      </div>
                       <div class="form-group">
                        <label for="exampleInputConfirmPassword1">Date Occupied</label>
                        <input type="date" class="form-control" id="exampleInputConfirmPassword1" value="<?php echo $date_occupied ?>" name="date_occupied">
                      </div>
                       <div class="form-group">
                        <label for="exampleInputConfirmPassword1">Collection of Rental</label>
                        <select class="form-control" value="<?php echo $name_of_occupant ?>" name="collection_of_rental">
                            <option hidden>Choose A Collection Rental</option>
                            <option value="<?php echo $collection_of_rental?>" hidden selected><?php echo $collection_of_rental?></option>
                            <option value="15">15 DAYS</option>
                            <option value="30">30 DAYS</option>
                        </select>
                      </div>
                      <button type="submit" class="btn btn-success btn-md mr-2" name="editAgreementFunc">Submit</button>
                    </form>
                  </div>
                </div>
              </div>
